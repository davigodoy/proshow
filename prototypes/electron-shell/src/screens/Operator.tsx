...
import {
  audioPresentationTheme,
  themeHasBackgroundArt,
} from '../theme/audioTheme'
...
function isSameAudioProgram(
  ...
  return Boolean(
    mediaPath &&
      live.kind === 'audio' &&
      (live.mediaKind === 'audio' || !live.mediaKind) &&
      live.mediaPath === mediaPath,
  )
}
...
export function Operator({
  ...
  /** Evita gravar [] no disco antes de restaurar o plano da sessão anterior. */
  ...
  const [mediaLibDragOver, setMediaLibDragOver] = useState(false)
  /** Arquivo do Finder sobre qualquer área do operador. */
  ...
  const previewItem = useMemo(() => {
    if (browsingItem) {
      ...
      // Mídia: preferir catálogo de mídia (tema/arte atualizados no select)
      if (browsingItem.kind !== 'lyrics' && browsingItem.kind !== 'bible') {
        const fromMedia = mediaLibrary.find(
          (s) => s.id === browsingItem.id || songKey(s) === key,
        )
  ...
  }, [plan, previewId, browsingItem, library, mediaLibrary])
  ...
  /**
   * Marca "5a", "5b", "6a"… para as partes do intervalo bíblico.
   *
   * O intervalo é repartido como um bloco só, então uma parte pode começar no
   * meio de um verso. O número é o verso real onde a parte COMEÇA; a letra é a
   * ordem da parte dentro desse verso.
   */
  const biblePartTags = useMemo(() => {
    const verses = biblePreview?.lines ?? []
    const verseNos =
      biblePreview?.verseNumbers?.length === verses.length
        ? biblePreview.verseNumbers
        : verses.map((_, i) => i + 1)
    ...
    return bibleDerived.lines.map((part) => {
      ...
      const verseNo =
        vIdx < 0
          ? (verseNos[verseNos.length - 1] ?? verses.length)
          : (verseNos[vIdx] ?? vIdx + 1)
  ...
  /** Mídia visual cobre o stage — áudio NÃO (câmera/tema/arte podem ficar atrás). */
  const previewMediaBlocksCamera =
    previewIsMedia &&
    previewItem?.kind !== 'audio' &&
    previewMediaKind !== 'audio'
  const liveMediaBlocksCamera = Boolean(
    liveMediaPath &&
      live.kind !== 'audio' &&
      liveMediaKind !== 'audio',
  )
  ...
  /** Arte do tema/mídia no preview de áudio — ganha da câmera de fundo. */
  const previewAudioTheme =
    previewItem?.kind === 'audio'
      ? audioPresentationTheme(previewItem, basePreviewTheme, themes)
      : null
  const previewAudioHasArt = themeHasBackgroundArt(previewAudioTheme)

  const previewCameraId = suppressPreviewCam
    ? null
    : usingBiblePreview
      ? activeCameraId
      : previewMediaBlocksCamera
        ? null
        : previewItem?.kind === 'camera'
          ? previewItem.cameraDeviceId || cameras[0]?.deviceId || null
          : previewItem?.kind === 'audio' && previewAudioHasArt
            ? null
            : activeCameraId

  // Painel AO VIVO: câmera do programa (device vivo ou fundo armado).
  // Áudio com arte de tema/mídia: câmera fica de fora (a arte tem prioridade).
  const liveStageTheme = (() => {
    let t = hydrateThemeMedia(
      resolveActiveTheme(live, theme, bibleTheme),
      themes,
    )
    // Recupera arte do tema se um override antigo gravou null (câmera)
    if (live.kind === 'audio' && !themeHasBackgroundArt(t)) {
      const id = live.themeOverride?.id || null
      const fromList = id ? themes.find((x) => x.id === id) : null
      if (themeHasBackgroundArt(fromList)) {
        t = {
          ...t,
          backgroundImage: fromList!.backgroundImage || null,
          backgroundVideo: fromList!.backgroundVideo || null,
        }
      } else if (themeHasBackgroundArt(theme) && (!id || id === theme.id)) {
        t = {
          ...t,
    ...
    // Câmera limpa arte na exibição — exceto áudio com arte do tema (prioridade)
    if (
      live.cameraDeviceId &&
      !(live.kind === 'audio' && themeHasBackgroundArt(t))
    ) {
      t = { ...t, backgroundImage: null, backgroundVideo: null }
    }
    // Tema AND portões AO VIVO
    t = andThemeShowGates(t, { ...live, showText: true })
    // Bíblia: checkbox "Referência" vale; o gate AO VIVO não apaga
    if (live.kind === 'bible') {
      t = {
        ...t,
  ...
  const liveAudioHasArt =
    live.kind === 'audio' && themeHasBackgroundArt(liveStageTheme)

  const liveOperatorCameraId = liveMediaBlocksCamera
    ? null
    : operatorCamOff
      ? null
      : liveAudioHasArt
  ...
  const previewStageTheme = (() => {
    ...
    if (usingBiblePreview) {
    ...
    } else if (previewItem?.kind === 'audio' && previewAudioTheme) {
      t = previewAudioTheme
    } else {
      const phraseSrcIdx =
      ...
      t = hydrateThemeMedia(
        withSongUppercase(
          resolvePhraseTheme(
            basePreviewTheme,
            themes,
            previewItem?.phraseStyles?.[phraseSrcIdx],
            previewItem?.themeId,
          ),
      ...
      if (previewItem?.kind === 'lyrics' && previewItem.bgMediaPath) {
        t = {
          ...t,
          backgroundImage:
            previewItem.bgMediaKind === 'image'
              ? previewItem.bgMediaPath
              : null,
          backgroundVideo:
            previewItem.bgMediaKind === 'video'
              ? previewItem.bgMediaPath
              : null,
        }
      }
    }
    // Câmera no preview esconde arte — exceto áudio com arte do tema
    if (previewCameraId && !(previewItem?.kind === 'audio' && themeHasBackgroundArt(t))) {
      t = { ...t, backgroundImage: null, backgroundVideo: null }
    }
    // Preview: portões AO VIVO apenas — o tema não esconde título/artista/letra.
    // Bíblia: referência segue o checkbox, ignora gateTitle.
    const textOn = !previewIsCamera
    if (usingBiblePreview || previewItem?.kind === 'bible') {
      t = {
        ...t,
        showTitle: bibleTheme.showTitle !== false,
        showArtist: false,
        showLyrics:
          bibleTheme.showLyrics !== false &&
          live.gateLyrics !== false &&
          textOn,
      }
    } else if (previewItem?.kind === 'audio') {
      // Áudio: só o fundo do tema (ou mídia de fundo); sem título/letra.
      t = {
        ...t,
        showTitle: false,
        showArtist: false,
      ...
      t = {
        ...t,
        showTitle: live.gateTitle !== false && textOn,
        showArtist: live.gateArtist !== false && textOn,
        showLyrics: live.gateLyrics !== false && textOn,
      }
    }
    return t
  })()
  ...
  const previewPhraseSourceIdx =
    previewDerived.sourceIndex[previewSlide] ?? previewSlide
  const currentPhraseStyle =
    previewItem?.phraseStyles?.[previewPhraseSourceIdx] || null
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const items = await api.planLoad?.()
        if (Array.isArray(items) && items.length) {
          setPlan(items as PlanItem[])
        }
      } catch {
        /* disco ilegível — começa vazio */
      } finally {
  ...
  // Persiste o plano do culto (debounce) — sobrevive a fechar o app.
  useEffect(() => {
    if (!planHydrated) return
    const api = window.projection
    if (!api?.planSave) return
    const t = window.setTimeout(() => {
      void api.planSave(plan).catch(() => {
        /* falha de disco — plano continua em RAM */
      })
    }, 450)
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      const inBibleJump = Boolean(el?.closest?.('.bible-jump-overlay'))
      const isNumberInput =
        tag === 'INPUT' &&
        (el as HTMLInputElement | null)?.type === 'number'

      // Bíblia: digitar abre a busca (livro na biblioteca; capítulo na coluna
      // do meio). Exceto De/Até e o próprio overlay de salto.
      if (
        tab === 'show' &&
        libTab === 'biblia' &&
        !inBibleJump &&
        ...
        if (!editableField || tag === 'SELECT') {
          e.preventDefault()
          const inMiddleColumn = Boolean(
            el?.closest?.('.operator-detail') || el?.dataset?.biblePart != null,
          )
          if (inMiddleColumn) bibleRef.current?.startJumpChapter(e.key)
          else bibleRef.current?.startJump(e.key)
          return
        }
      }
      ...
      if (e.key === 'Escape') {
        ...
        // Fecha a exibição (black) e corta áudio de fato
        onLiveChange({
          visible: false,
          mediaPlaying: false,
          mediaMuted: true,
  ...
  async function takeLive(
    ...
    if (item.kind === 'camera') {
      ...
      const camLabel =
        cameras.find((c) => c.deviceId === deviceId)?.label || item.label || ''
      if (/ndi|virtual\s*input/i.test(camLabel)) {
        toastInfo(
          'NDI Virtual Input: deixe a janela do Virtual Input visível (outro monitor ajuda). Se cobrir, a câmera congela.',
        )
      }
    ...
    if (
      ...
      // Mídia pura (imagem/vídeo/áudio/deck/web): título do tema = nome do
      // arquivo, renderizado no centro — esconde na saída.
      const mediaHidesMeta =
        item.kind === 'image' ||
        item.kind === 'video' ||
        item.kind === 'audio' ||
        item.kind === 'deck' ||
        item.kind === 'web' ||
        item.kind === 'file'
      // Áudio: arte do tema/mídia tem prioridade; câmera só se não houver arte.
      const audioTheme =
        item.kind === 'audio'
          ? audioPresentationTheme(item, theme, themes)
          : null
      const camId =
        item.kind === 'audio' &&
        bgCameraEnabled &&
        !themeHasBackgroundArt(audioTheme)
          ? bgCameraId ||
            (live.kind !== 'camera' ? live.cameraDeviceId || null : null)
          : null
      if (camId) setBgCameraId(camId)

      const sameAudio = isSameAudioProgram(live, deckPath)

      // Mesmo MP3 já no ar: só troca tema/câmera — não reinicia a música
      // (forceRestart = duplo clique / reabrir do zero)
      if (
        sameAudio &&
        item.kind === 'audio' &&
        audioTheme &&
        ...
        if (camId && !live.cameraDeviceId && !simulation) {
          setOperatorCamOff(true)
          releaseAllCameraStreams()
          await sleep(220)
        } else if (!camId) {
      ...
      await onLiveChange({
        ...
        mediaVoiceIsolate: isAv
          ...
          : false,
        themeOverride: mediaHidesMeta
          ? item.kind === 'audio' && audioTheme
            ? audioTheme
            : {
                ...(live.kind === 'bible' ? bibleTheme : theme),
                showTitle: false,
    ...
    // Com câmera de fundo: libera streams do operador; a arte do tema
    // permanece no override — o LyricStage não monta imagem/vídeo com showCam.
    if (camId) {
      if (!simulation) {
        setOperatorCamOff(true)
        releaseAllCameraStreams()
  ...
  function clearCameraBackground() {
    ...
    if (live.kind === 'camera') return
    // Re-hidrata arte do tema (versões antigas gravavam null ao ligar a câmera)
    let themeOverride = live.themeOverride
    if (themeOverride?.id) {
      const fromList = themes.find((t) => t.id === themeOverride!.id)
      if (fromList) {
        themeOverride = {
          ...themeOverride,
          backgroundImage:
            themeOverride.backgroundImage ||
            fromList.backgroundImage ||
            null,
          backgroundVideo:
            themeOverride.backgroundVideo ||
            fromList.backgroundVideo ||
            null,
        }
      }
    }
  ...
  async function takeLiveAndAdvance() {
    ...
    if (browsingItem && songKey(browsingItem) === songKey(item)) {
      // `item` (previewItem) já traz tema/arte frescos do catálogo
      const committed = selectLibraryItem(item)
      setBrowsingItem(null)
      item = committed ?? item
    }
  ...
  function selectLibraryItem(item: PlanItem) {
    ...
    if (existing) {
      // Preferir metadados frescos da biblioteca (tema/arte, PDF re-rasterizado…)
      const merged: PlanItem = {
        ...existing,
        themeId: item.themeId !== undefined ? item.themeId : existing.themeId,
        uppercase:
          item.uppercase !== undefined ? item.uppercase : existing.uppercase,
        bgMediaPath:
          item.bgMediaPath !== undefined
            ? item.bgMediaPath
            : existing.bgMediaPath,
        bgMediaKind:
          item.bgMediaKind !== undefined
            ? item.bgMediaKind
            : existing.bgMediaKind,
  ...
  /** Tema / maiúsculas — letras (tema+maiúsc.); áudio só tema (sem maiúsc.).
   * Só Preview + memória; AO VIVO no próximo Enviar (sem reiniciar se for o mesmo MP3). */
  function applySongPresentation(
    ...
    if (item.kind === 'audio') {
      const audioPatch = {
        ...
        uppercase: null as null,
      }
      const key = songKey(item)
      patchSongInLibraryAndPlan(key, audioPatch)
      setMediaLibrary((lib) =>
        lib.map((s) =>
          s.id === item.id || songKey(s) === key ? { ...s, ...audioPatch } : s,
        ),
      )
      setPlan((p) =>
        p.map((x) => (x.id === item.id ? { ...x, ...audioPatch } : x)),
      )
      return
    }
    if (item.kind !== 'lyrics') return
  ...
  /**
   * Arte de fundo (imagem/vídeo) — letras e áudio (mp3/wav/m4a/…).
   * Só Preview + memória; AO VIVO no próximo Enviar (mesmo MP3 = só troca o fundo).
   */
  function applySongBackground(
    item: PlanItem,
    patch: { bgMediaPath?: string | null; bgMediaKind?: 'image' | 'video' | null },
  ) {
    if (item.kind !== 'lyrics' && item.kind !== 'audio') return
    const key = songKey(item)
    patchSongInLibraryAndPlan(key, patch)
    setMediaLibrary((lib) =>
      lib.map((s) =>
        s.id === item.id || songKey(s) === key ? { ...s, ...patch } : s,
      ),
    )
  ...
  /** Estilo da frase no Preview — indexado pela linha escrita (sourceIndex).
   * Não empurra AO VIVO: só entra no ar no próximo takeLive dessa linha. */
  function setPhraseStyle(partial: PhraseStyle | null) {
    if (!previewItem || previewItem.kind !== 'lyrics') return
    const srcIdx = previewDerived.sourceIndex[previewSlide] ?? previewSlide
    const styles = [...(previewItem.phraseStyles || [])] as Array<
      PhraseStyle | null
    >
    while (styles.length <= srcIdx) styles.push(null)
    styles[srcIdx] = partial
  ...
  async function toggleProgramLive() {
    ...
    if (next) {
      ...
      onLiveChange({
        visible: true,
        // Volta ao ar: retoma mídia se houver vídeo/áudio/web
        mediaPlaying:
          live.kind === 'video' ||
          live.kind === 'audio' ||
          live.kind === 'web' ||
  ...
  async function dropFilesOnPlan(fileList: FileList | File[]) {
    ...
    const mapped = mapImportedMedia(result?.items || [])
    if (!mapped.length) {
      setMediaNote(result?.note || 'Nenhum arquivo de mídia reconhecido')
      return
    }
    setLibTab('midia')
    setMediaLibrary((prev) => [...mapped, ...prev])
  ...
  /** Arrastar arquivos do Finder → catálogo Mídia + Preview (sem plano). */
  async function dropFilesOnMediaLibrary(fileList: FileList | File[]) {
    const files = Array.from(fileList)
    const paths: string[] = []
    for (const file of files) {
      const p = window.projection?.getPathForFile?.(file)
    ...
    const notes = [
      result?.note,
      ...(result?.items || []).map((i) => i.note).filter(Boolean),
    ].filter(Boolean)
    if (notes.length) setMediaNote(notes.join(' · '))
  }

  /** Drag de arquivos do SO (Finder) — ignora arrastes internos do app. */
  function isOsFileDrag(e: React.DragEvent) {
    const types = e.dataTransfer?.types
    if (!types) return false
    const list = Array.from(types)
    if (!list.includes('Files')) return false
    // Reordenação / item do plano ou biblioteca
    if (
      list.includes('application/x-ible-plan') ||
      list.includes('application/x-ible-plan-reorder') ||
      list.includes('application/x-proshow-lines')
    ) {
      return false
    }
    return true
  ...
  return (
    <div
      className={`op${live.visible ? ' is-program-live' : ' is-program-black'}${
        fileDropOver ? ' file-drop-target' : ''
      }`}
      onDragOver={(e) => {
        if (!isOsFileDrag(e) || planDragId) return
        e.preventDefault()
      ...
      onDragLeave={(e) => {
        ...
        setMediaLibDragOver(false)
      }}
      onDrop={(e) => {
        if (!isOsFileDrag(e) || !e.dataTransfer.files?.length) return
        e.preventDefault()
        setFileDropOver(false)
        setMediaLibDragOver(false)
        void dropFilesOnMediaLibrary(e.dataTransfer.files)
      }}
    >
      ...
      {tab === 'show' ? (
        <div className="op-grid">
          <section
            className={`panel library${mediaLibDragOver ? ' drop-target' : ''}`}
            ref={libraryZoneRef}
            onDragOver={(e) => {
              ...
              e.stopPropagation()
              e.dataTransfer.dropEffect = 'copy'
              setMediaLibDragOver(true)
              setFileDropOver(true)
              if (libTab !== 'midia') setLibTab('midia')
            }}
            onDragLeave={(e) => {
              if (e.currentTarget.contains(e.relatedTarget as Node)) return
              setMediaLibDragOver(false)
                  ...
                  <ul>
                    {bibleDerived.lines.length > 1
                          ...
                          return (
                            <li key={`bible-part-${idx}`}>
                              <button
                                ...
                                title={`${tag || `Parte ${idx + 1}`} — 1 clique arma, 2 cliques projetam`}
                              >
                                <span className="phrase-num">{tag || idx + 1}</span>
                      ...
                      : biblePreview.lines.map((line, idx) => {
                          const verseNo =
                            biblePreview.verseNumbers?.[idx] ?? idx + 1
                          return (
                            <li key={`bible-v-${idx}`}>
                              <div className="phrase bible-verse-row">
                                <span className="phrase-num">{verseNo}</span>
              ...
              <div className="stage-column stage-preview-column">
                <div className="stage-block stage-preview">
                  ...
                  {previewItem?.kind === 'lyrics' ? (
                    ...
                    <div className="preview-style-bar preview-style-bar-audio">
                      <label className="preview-style-field">
                        <span>Tema</span>
                        <select
                          key={`audio-theme-${themesRev}`}
                          value={previewItem.themeId || ''}
                          onChange={(e) =>
                            applySongPresentation(previewItem, {
                              themeId: e.target.value || null,
                      ...
                      <div className="song-bg-media-group">
                        <button
                          type="button"
                          className="song-bg-btn song-bg-btn-add"
                          title="Imagem ou vídeo de fundo (tela cheia)"
                          onClick={() => void pickSongBackground(previewItem)}
                        >
                          + Mídia de fundo
                        </button>
