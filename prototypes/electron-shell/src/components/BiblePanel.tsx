...
export type BibleItem = {
  ...
  title?: string
  lines: string[]
  /** Números reais dos versículos (paralelo a `lines`). */
  verseNumbers?: number[]
  source?: string
}
...
export type BiblePanelHandle = {
  /** Desliza a janela De–Até no capítulo atual. Na borda, não troca de capítulo. */
  step: (delta: number) => boolean
  /** Projeta a janela atual e avança; no fim do capítulo, vai ao próximo. */
  liveAndAdvance: () => void
  ...
  /** Abre a busca no capítulo do livro atual (coluna do meio). Esc volta ao livro. */
  startJumpChapter: (char?: string) => void
}
...
export const BiblePanel = forwardRef<BiblePanelHandle, Props>(function BiblePanel(
  ...
  async function buildItem(rangeFrom = from, rangeTo = to): Promise<BibleItem | null> {
    ...
    const lines = range.slides || range.lines || []
    const verseNumbers =
      Array.isArray(range.verses) && range.verses.length
        ? range.verses.map((v) => v.n)
        : lines.map((_, i) => rangeFrom + i)
    return {
  ...
  useImperativeHandle(
    ...
    () => ({
      step(delta: number) {
        // Só desliza De–Até no capítulo atual — Enter/↑/↓ na borda não trocam capítulo.
      ...
      liveAndAdvance() {
        ...
        // Fim do capítulo (ou do último verso): Enter abre o próximo capítulo.
        const book = books[bookIndex]
        if (!book) return
        ...
        const nextBook = bookIndex + 1
        if (nextBook >= books.length) return
        pendingVerseRef.current = 1
      ...
      startJumpChapter(char = '') {
        ...
        setJumpBookIdx(book.index)
        setJumpBookQuery(book.name)
        setJumpChapterQuery(/^\d$/.test(char) ? char : '')
        setJumpChapterNum(null)
        setJumpVerseQuery('')
        setJumpVerseMax(null)
        setJumpStage('chapter')
        setJumpOpen(true)
        requestAnimationFrame(() => {
          const input = jumpChapterInputRef.current
          if (!input) return
          input.focus()
          input.setSelectionRange(input.value.length, input.value.length)
        })
      ...
      <div className="bible-row">
        ...
        <label>
          ...
          <select
            ...
            onChange={(e) => setChapterIndex(Number(e.target.value) - 1)}
          >
