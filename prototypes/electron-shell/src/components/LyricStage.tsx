...
export function LyricStage({
  ...
  const hasVideo = Boolean(mediaUrl && (mediaKind === 'video' || mediaKind === 'audio'))
  const hasWebEmbed = Boolean(
    mediaKind === 'web' && mediaUrl && /^https?:\/\//i.test(mediaUrl),
  )
  const webMuted = Boolean(mediaForceMuted || mediaPlayback.muted)
  ...
  useEffect(() => {
    ...
    const volume = Math.round(
      Math.max(0, Math.min(1, mediaPlayback.volume ?? 1)) * 100,
    )
    const apply = () => {
      ...
      youtubeCommand(
        win,
        mediaPlayback.playing === false ? 'pauseVideo' : 'playVideo',
      )
    }
    apply()
  ...
  const showCam = Boolean(showCamera && cameraDeviceId && !hasMediaBg)
  ...
  const artisticPlanKey =
    useSlots && resolvedArtisticPlan
      ...
      : ''
  const wantTitle = Boolean(theme.showTitle && title && !useSlots)
  const wantArtist = Boolean(theme.showArtist && artist && !useSlots)
  const deckTextOnly =
  ...
  /** Dispara fade via preto ao trocar letra / bíblia / mídia / câmera */
  ...
  // Fade via preto ao trocar letra ↔ bíblia ↔ mídia ↔ câmera
  useLayoutEffect(() => {
    if (contentFamilyRef.current === contentFamily) return
    contentFamilyRef.current = contentFamily
    if (!visible) return
    setFamilyFadeGen((n) => n + 1)
  }, [contentFamily, visible])
      ...
      <div className={`lyric-stage-program ${programOn ? 'is-on' : 'is-off'}`}>
        ...
        {hasVideo ? (
          <MediaPlayer
            ...
            forceMuted={mediaForceMuted}
            mediaFit={mediaFit}
            className="lyric-stage-media-player"
            onTime={onMediaTime}
            ensureAudioGraph={Boolean(
              spectrumEffective?.enabled && spectrumEffective.source === 'media',
            )}
          />
        ) : null}
