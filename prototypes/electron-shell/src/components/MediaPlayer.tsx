...
export function MediaPlayer({
  ...
  /** Este player registrou o tap global do espectro — só ele pode limpar. */
  const ownsSpectrumTapRef = useRef(false)
  ...
  useEffect(() => {
    return () => {
      ...
      if (ownsSpectrumTapRef.current) {
        setMediaSpectrumAnalyser(null)
        ownsSpectrumTapRef.current = false
      }
      try {
  ...
  async function wireAudioGraphOnce(isolate: boolean) {
    ...
    try {
      ...
      // Tap ANTES do gain: espectro continua lendo o sinal mesmo com mute
      // (preview/operador espelho emudecem só os speakers).
      ...
      // Só o player com áudio real registra o tap global — senão o preview
      // mudo sobrescreve o AO VIVO e o espectro some sobre a mídia.
      if (!forceMutedRef.current) {
        setMediaSpectrumAnalyser(spectrumAnalyser)
        ownsSpectrumTapRef.current = true
      } else if (ownsSpectrumTapRef.current) {
        setMediaSpectrumAnalyser(null)
        ownsSpectrumTapRef.current = false
      }
...
function IconRestart() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path
        d="M12 5V2.5L8.5 6 12 9.5V7a5 5 0 1 1-5 5H5a7 7 0 1 0 7-7z"
        fill="currentColor"
      />
    </svg>
  )
...
function IconMute() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path
        d="M5 9v6h3.5L13 19.5V4.5L8.5 9H5z"
        fill="currentColor"
      />
      <path
        d="M16.2 9.8l1.4 1.4 1.4-1.4 1.1 1.1-1.4 1.4 1.4 1.4-1.1 1.1-1.4-1.4-1.4 1.4-1.1-1.1 1.4-1.4-1.4-1.4 1.1-1.1z"
        fill="currentColor"
      />
    </svg>
  )
...
function IconVolume() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M5 9v6h3.5L13 19.5V4.5L8.5 9H5z" fill="currentColor" />
      <path
        d="M15.2 9.3a3.5 3.5 0 0 1 0 5.4l-1.15-.9a2.1 2.1 0 0 0 0-3.6l1.15-.9zm2.35-2.55a6.5 6.5 0 0 1 0 10.5l-1.1-.95a5.1 5.1 0 0 0 0-8.6l1.1-.95z"
        fill="currentColor"
      />
    </svg>
  )
...
export function MediaTransport({
  ...
  const controlButtons = (
    <div className="media-transport-btns">
      ...
      <button
        type="button"
        className="media-transport-btn primary"
        title={playback.playing ? 'Pausar' : 'Reproduzir'}
      ...
      <button
        type="button"
        className="media-transport-btn"
        title={playback.muted ? 'Ativar som' : 'Mudo'}
        aria-label={playback.muted ? 'Ativar som' : 'Mudo'}
        onClick={() => onChange({ muted: !playback.muted })}
      >
        {playback.muted ? <IconMute /> : <IconVolume />}
      </button>
    </div>
  )
  ...
  return (
    <div className="media-transport">
      <div className="media-transport-top">
        ...
        <div className="media-transport-btns-wrap">
          {controlButtons}
          <label
