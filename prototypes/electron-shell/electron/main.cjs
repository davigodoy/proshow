...
// Câmeras virtuais (NDI Virtual Input) e vídeo em janela coberta/oculta:
// sem isto o Chromium suspende o decode e a captura “trava”.
...
function createOperator() {
  const saved = prefs.operatorBounds;
  const opts = {
    width: 1280,
    height: 800,
    minWidth: 1000,
  ...
  operatorWin = new BrowserWindow(opts);
  try {
    operatorWin.webContents.setBackgroundThrottling(false);
  } catch {
    /* ignore */
  }

  // Se o monitor sumiu, centraliza no display principal.
  if (saved) {
    try {
      const bounds = operatorWin.getBounds();
  ...
  operatorWin.once("ready-to-show", () => {
    ...
    if (saved?.isMaximized) operatorWin.maximize();
    operatorWin.show();
  });

  let saveBoundsTimer = null;
  function persistOperatorBounds() {
    if (!operatorWin || operatorWin.isDestroyed()) return;
    const isMaximized = operatorWin.isMaximized();
    // Em maximizado, getBounds() pode ser a área útil — guarda o normal.
    const bounds = isMaximized
      ? operatorWin.getNormalBounds?.() || operatorWin.getBounds()
      : operatorWin.getBounds();
  ...
  function schedulePersistOperatorBounds() {
    if (saveBoundsTimer) clearTimeout(saveBoundsTimer);
    saveBoundsTimer = setTimeout(persistOperatorBounds, 250);
  }

  operatorWin.on("move", schedulePersistOperatorBounds);
  operatorWin.on("resize", schedulePersistOperatorBounds);
  operatorWin.on("maximize", persistOperatorBounds);
  operatorWin.on("unmaximize", persistOperatorBounds);
...
function ensureNdiCaptureWindow() {
  ...
  ndiCaptureWin = new BrowserWindow({
    ...
    webPreferences: {
      ...
      contextIsolation: true,
      nodeIntegration: false,
      // offscreen:true corta o avanço de <video>/getUserMedia (NDI Virtual Input).
      // Mantém janela oculta, mas com compositor ativo o suficiente pra câmera.
      backgroundThrottling: false,
    },
  });

  try {
    ndiCaptureWin.webContents.setBackgroundThrottling(false);
  } catch {
    /* ignore */
  }
  try {
    // Ajuda capturePage a manter ritmo mesmo com show:false
    ndiCaptureWin.webContents.setFrameRate?.(30);
  } catch {
    /* ignore */
  }
...
function createOutput() {
  ...
  try {
    outputWin.webContents.setBackgroundThrottling(false);
  } catch {
    /* ignore */
  }
...
/** Linha placeholder de mídia/câmera — não vai pro overlay OBS. */
function isOverlayPlaceholderLine(line) {
  return /^\[.+\]$/.test(String(line || "").trim());
}

/**
 * Overlay OBS: só texto de letra/bíblia. Sem status, sem "[ imagem ]",
 * sem placeholder de câmera/mídia — mídia pura fica em silêncio tipográfico.
 */
function overlayDisplayLines(state) {
  ...
  if (state?.stackArtistic && Array.isArray(slots) && slots.length) {
    ...
    return String(last)
      .split(/\n/)
      .map((s) => s.trim())
      .filter((s) => s && !isOverlayPlaceholderLine(s));
  }
...
/** Título no overlay: só letra/bíblia (referência ou nome da música). */
function overlayDisplayTitle(state) {
  if (!state || state.stackArtistic) return "";
  const kind = state.kind;
  if (
    kind === "image" ||
    kind === "video" ||
...
<body class="${live.visible ? "" : "hide"}">
  <div class="wrap"><div class="card">
    <div class="title">${escapeHtml(overlayDisplayTitle(live))}</div>
    ${lines}
  </div></div>
  <script>
    function esc(s){
    ...
    function overlayTitle(j){
      if(!j||j.stackArtistic) return '';
      var k=j.kind;
      if(k==='image'||k==='video'||k==='audio'||k==='camera'||k==='file'||k==='deck'||k==='web')
        return '';
      return j.title||'';
    }
    function overlayLines(j){
      ...
      if(k==='image'||k==='video'||k==='audio'||k==='camera'||k==='file'||k==='deck'||k==='web')
        return [];
      var slots=j.phraseSlots;
      if(j.stackArtistic&&Array.isArray(slots)&&slots.length){
    ...
    async function tick(){
      try{
        ...
        document.querySelector('.card').innerHTML=
          '<div class="title">'+esc(overlayTitle(j))+'</div>'+
          overlayLines(j).map(function(l){return '<div class="line">'+esc(l)+'</div>';}).join('');
      }catch(e){}
    }
