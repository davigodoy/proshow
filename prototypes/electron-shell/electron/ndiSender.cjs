...
async function runLoop() {
  ...
  while (enabled && sender) {
    ...
    try {
      ...
      // Sem timeout, capturePage pode travar o loop (e a UI) se a câmera NDI
      // Virtual Input parar de entregar frames com a janela coberta.
      const img = await withTimeout(wc.capturePage(), 450, "capturePage");
